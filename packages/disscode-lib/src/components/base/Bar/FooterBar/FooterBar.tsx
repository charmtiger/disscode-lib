...
const FooterBar = observer(({ current = 0, list = [], color, selectedColor, style, onChange }: FooterBarProps) => {
  ...
  return (
    <View>
          ...
          return (
            ...
            >
              <View className=' M-flex-item M-vertical-bottom M-horizontal-center'>
                <View className='cs-footer__iconContainer'>
                  <Image
                    className={`cs-footer__icon ${isSelect ? '' : 'M-display-none'}`}
                    src={item.selectedIconPath || require('@/assets/images/default_img.png')}
                    style={{
                  ...
                  <Image
                    className={`cs-footer__icon ${!isSelect ? '' : 'M-display-none'}`}
                    src={item.iconPath || require('@/assets/images/default_img.png')}
                    style={{
                      width: px((item?.width || 0) * 2),
                      height: px((item?.height || 0) * 2),
                    }}
      ...
      <SafeView forceInset={{ top: 'never', bottom: 'always' }} style={{ backgroundColor: '#fff' }} />
    </View>
  );
});
